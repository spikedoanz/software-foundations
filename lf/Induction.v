...
Theorem plus_succ : forall n m : nat,
  S (n + m) = n + (S m).
Proof.
  intros n. induction n as [|n' IHn'].
  intros m.
...
Proof.
  ...
  - simpl. rewrite hn. rewrite plus_succ. reflexivity. 
Qed.
...
Proof.
  ...
  induction n as [|n hn].
  - simpl. reflexivity.
  - rewrite hn. rewrite negation_fn_applied_twice.
    + simpl. reflexivity. 
    + reflexivity. 
Qed.
...
(** **** Exercise: 2 stars, advanced, optional (add_comm_informal)
    ...
    ```
    ------------------------------------------------------------
    to show n + m = m + n, we use induction.

      first, assume n = 0, we must show
      0 + m = m + 0
      simplify, we get
      m = m, 
      which is true reflexively

      second, we assume the hypothesis is true for an arbitrary n'
      thus, we have the inductive hypothesis
      hn' : n' + m = m + n'
      and we need to show
      S n' + m = m + S n'
      we use our inductive hypothesis, obtaining:
      S m + n' = m + S n'
      we previously showed that S can be rebracketed across addition
      m + S n' = m + S n'
      which is true reflexively
    Qed.
*)
...
(** **** Exercise: 2 stars, standard, optional (eqb_refl_informal)
    ...
    in the case that n is 0, since 0=0 this is true trivially.
    in the case that n is the successor of some number k, 
    S k = S k is also true trivially.
...
Theorem mul_succ : forall n m : nat,
  n * S m = n * m + n.
Proof.
  intros.
  induction n as [|n hn].
...
Proof.
  ...
  - simpl. rewrite mul_succ. rewrite add_comm. rewrite hn. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - rewrite mul_succ. rewrite hn. rewrite plus_1_r. reflexivity.
Qed.
...
Theorem mul_plus_distr_r : forall n m p : nat,
  (n + m) * p = (n * p) + (m * p).
Proof.
  intros.
  induction p as [|p hp].
  - rewrite mul_0_r. rewrite mul_0_r. rewrite mul_0_r.
    simpl. reflexivity.
  - rewrite mul_succ. rewrite mul_succ. rewrite mul_succ. 
    rewrite hp. rewrite add_assoc. rewrite add_assoc.
    rewrite (add_comm (n * p + m * p) n). rewrite add_assoc.
    rewrite (add_comm (n * p) n).
    reflexivity.
Qed.

Theorem mul_plus_distr_l : forall m n p: nat,
  n * (m + p) = (n * m) + (n * p).
Proof.
  intros.
  rewrite mul_comm.
  rewrite mul_plus_distr_r.
...
Proof.
  ...
  - rewrite mul_succ. rewrite mul_succ.
    rewrite mul_plus_distr_l.
    rewrite hp.
    reflexivity.
Qed.
...
Theorem succ_eq_plus_1_r : forall n : nat,
  S n = n + 1.
Proof.
  intros.
  rewrite add_comm.
...
Proof.
  ...
  - simpl. reflexivity.
  - simpl. rewrite hb'.
    simpl. rewrite plus_succ. rewrite plus_succ. rewrite plus_succ.
    reflexivity.
Qed.
...
Theorem double_bin_zero : double_bin Z = Z.
...
Lemma double_add : forall a b : nat,
  double (a + b) = a + b + a + b.
Proof.
  induction a as [|a ha].
  - simpl. induction b as [|b hb].
    + simpl. reflexivity.  
    + simpl. rewrite plus_n_Sm. 
      rewrite succ_eq_plus_1_r. rewrite succ_eq_plus_1_r.
      rewrite (succ_eq_plus_1_r (S b)). rewrite (succ_eq_plus_1_r (b)).
      rewrite hb. rewrite add_assoc. rewrite add_assoc. reflexivity.
  - simpl. intros b. rewrite ha.
...
Lemma bin_to_nat_b0_eq_double_b : forall b, 
  bin_to_nat (B0 b) = double (bin_to_nat b).
Proof.
  induction b as [|b hb |b hb].
  - simpl. reflexivity. 
  - simpl. rewrite double_add.
...
Lemma double_bin_incr_eq_incr_incr_double_bin :
...
Lemma nat_to_bin_double_eq_double_nat_to_bin :
  forall n :nat , nat_to_bin (double n) = double_bin (nat_to_bin n).
Proof.
  induction n as [|n hn].
  - simpl. reflexivity.
  - simpl. rewrite hn.
    rewrite double_bin_incr_eq_incr_incr_double_bin.
    reflexivity.
Qed.

(*
Lemma double_bin_normalize_eq_double_bin :
  forall b : bin, double_bin (normalize b) = double_bin b.
Proof.
  induction b as [|b hb |b hb].
  - simpl. reflexivity.
  - simpl.
...
Proof.
  ...
  induction b as [|b hb |b hb].
  - simpl. reflexivity. 
  - rewrite bin_to_nat_b0_eq_double_b. simpl.
    ...
    + reflexivity.
    + reflexivity.
  - simpl. rewrite self_double_add.
    rewrite nat_to_bin_double_eq_double_nat_to_bin.
    rewrite hb.
    rewrite incr_double_bin_eq_b1.
    reflexivity.
Qed.


(* wow that was a lot of fucking work *)
(** [] *)
